/*
 * Created on Nov 17, 2005
 */
package uk.org.ponder.darwin.rsf.producers;

public class ComponentIDs {
  public static final String MESSAGEFIELD = "messagefield";
  public static final String FRAME_TITLE = "frame-title";
  public static final String NAV_FRAME = "nav-frame";
  public static final String TEXT_TARGET = "text-target:";
  public static final String IMAGE_TARGET = "image-target:";
  public static final String PAGE_SELECT = "page-select";
  public static final String CURRENT_PAGE = "current-page";
  public static final String LAST_PAGE = "last-page";
  public static final String VIEW_TYPE = "view-type";
  
  public static final String FRAMES_PREFIX = "frames:";
  public static final String FRAMES_SIDE = "frames:side";
  public static final String FRAMES_IMAGE = "frames:image";
  public static final String FRAMES_TEXT = "frames:text";
  
  public static final String TEXT_FRAME = "txt-frame";
}
